...
class StockSelectionPolicy(SecuritySelectionPolicy):
    ...
    def __post_init__(self):
        if self.ignore_new_stock_period:
            ...
            self.ignore_new_stock_period = None if pd.isnull(self.ignore_new_stock_period) or np.isnan(self.ignore_new_stock_period) else int(self.ignore_new_stock_period)
        if self.select_new_stock_period:
            # self.select_new_stock_period = int(self.select_new_stock_period)
            self.select_new_stock_period = None if pd.isnull(self.select_new_stock_period) or np.isnan(self.select_new_stock_period) else int(self.select_new_stock_period)
